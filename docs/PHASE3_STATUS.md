...
Same pattern: domain logic **built and tested**; the on-device SLM and the cloud Gemini client are **injected via ports**; the results UI is left as Expo work.

## What runs and is tested now (Node/TypeScript)

| Capability | Where | Maps to |
|---|---|---|
| Shared result schema (both tiers) + runtime validator + JSON Schema for Gemini | `contractscan-core/schema.ts` | REQ-CONTRACT-006, §6.2 |
...
| Tier 2 proxy logic: consent → entitlement/quota → Gemini → validate → meter | `backend/lib/contractscan/analyze.ts` | §6.3, REQ-CONTRACT-005/012 |
...
- **One schema, two engines.** Tier 1 (on-device SLM) and Tier 2 (cloud Gemini) emit the **identical** `ContractAnalysis` shape, so the results screen is engine-agnostic. Tier 2 passes it to Gemini as `responseSchema` for guaranteed-valid JSON; Tier 1 constrains its SLM with a GBNF grammar built from the same shape. Both are re-validated at runtime. The provider sits behind a provider-neutral `CloudContractAnalyzer` port, so swapping models is a one-file change.
...
- **`CloudContractAnalyzer`** (Tier 2) — `backend/app/api/contractscan/analyze/route.ts` is the real route shell; the adapter wraps the Google GenAI SDK `@google/genai` (model `gemini-2.5-pro` from config, **paid tier/Vertex**, `responseSchema` = `CONTRACT_ANALYSIS_SCHEMA`, SSE streaming, in-memory only, no Search/Maps grounding or explicit caching). **Blocking pre-launch:** the per-project Gemini ZDR / retention decision in `docs/DECISIONS.md` #7 governs the consent-screen wording.
...
- [x] Tier 2 proxy: consent + entitlement + 2/month quota + validation + metering (logic; Gemini mocked)
- [x] Verdict escalation + legal disclaimer
- [ ] SLM + Gemini adapters wired; ≥20-contract eval set meets the recall bar
- [ ] Per-project Gemini ZDR (paid tier) — or accurate-disclosure copy — DECISIONS #7
